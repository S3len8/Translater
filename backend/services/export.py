...
class ExportService:
    ...
    async def create_excel(self):
        ...
        result = self._build_excel(histories)
        return result

    async def create_excel_for_range(self, start: int, end: int):
        histories = await self.repository.get_histories_by_range(start=start, end=end)
        result = self._build_excel(histories)
        return result
